...
{    public function index()
    ...
    }    public function store(Request $request)
    {
        ...
        }        $data = $request->all();        // Handle logo upload
        if ($request->hasFile('logo')) {
            ...
            \Log::info('Logo upload attempt:', [
                ...
                'client_extension' => $logoFile->getClientOriginalExtension(),
                'target_directory' => storage_path('app/public/logos'),
                'directory_exists' => is_dir(storage_path('app/public/logos')),
                'directory_writable' => is_writable(storage_path('app/public/logos'))
            ]);
              try {
                ...
                $success1 = copy($logoFile->getPathname(), $storagePath);
                \Log::info('Direct copy attempt:', ['success' => $success1, 'from' => $logoFile->getPathname(), 'to' => $storagePath]);
                
                if (!$success1) {
                    // Method 2: Laravel Storage facade
                    $fileContent = file_get_contents($logoFile->getPathname());
                    $success2 = Storage::disk('public')->put('logos/' . $logoName, $fileContent);
                    \Log::info('Storage facade attempt:', ['success' => $success2 !== false]);
                    
                    if (!$success2) {
                        // Method 3: move_uploaded_file
                        $success3 = $logoFile->move(storage_path('app/public/logos'), $logoName);
                        \Log::info('Move uploaded file attempt:', ['success' => $success3 !== false]);
                    }
                }
                
                // Verify file exists after all attempts
                ...
                \Log::info('Final file verification:', [
                  ...
                  if ($fileExists) {
                    ...
                    \Log::error('All upload methods failed for logo');
                    $data['logo'] = null;
                }
            } catch (\Exception $e) {
