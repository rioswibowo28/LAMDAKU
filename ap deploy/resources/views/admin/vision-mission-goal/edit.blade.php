...
@section('title', 'Edit Visi/Misi/Tujuan - LAMDAKU Admin')
@section('page-title', 'Edit Visi, Misi & Tujuan')
    ...
    <div class="col-md-8">
        <div class="card">
            <div class="card-header">
                <h5><i class="fas fa-edit me-2"></i>Edit Item</h5>
            </div>
            <div class="card-body">
                <form action="{{ route('admin.vision-mission-goal.update', $visionMissionGoal) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" required>
                            ...
                            @foreach($types as $key => $label)
                                <option value="{{ $key }}" {{ old('type', $visionMissionGoal->type) == $key ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                        @error('type')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('title') is-invalid @enderror" 
                               id="title" name="title" value="{{ old('title', $visionMissionGoal->title) }}" required>
                        @error('title')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('content') is-invalid @enderror" 
                                  id="content" name="content" rows="4" required>{{ old('content', $visionMissionGoal->content) }}</textarea>
                        @error('content')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('description') is-invalid @enderror" 
                                  id="description" name="description" rows="2">{{ old('description', $visionMissionGoal->description) }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control @error('icon_class') is-invalid @enderror" 
                                       id="icon_class" name="icon_class" value="{{ old('icon_class', $visionMissionGoal->icon_class) }}" 
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="color" class="form-control form-control-color @error('background_color') is-invalid @enderror" 
                                       id="background_color" name="background_color" value="{{ old('background_color', $visionMissionGoal->background_color) }}">
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control @error('sort_order') is-invalid @enderror" 
                                       id="sort_order" name="sort_order" value="{{ old('sort_order', $visionMissionGoal->sort_order) }}" min="1">
                                @error('sort_order')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                <div class="form-check mt-4">
                                    <input class="form-check-input" type="checkbox" id="is_active" name="is_active" 
                                           {{ old('is_active', $visionMissionGoal->is_active) ? 'checked' : '' }}>
                    ...
                    <div class="card-body" style="background: linear-gradient(135deg, #4285f415, #4285f405);">
                        <div class="d-flex align-items-start">
                            ...
                            <div class="flex-grow-1">
                                <h6 class="fw-bold mb-2" id="preview-title">{{ $visionMissionGoal->title }}</h6>
                                <p class="mb-1" id="preview-content">{{ $visionMissionGoal->content }}</p>
                                <small class="text-muted" id="preview-description">{{ $visionMissionGoal->description }}</small>
                            </div>
                        </div>
                    </div>
                </div>
